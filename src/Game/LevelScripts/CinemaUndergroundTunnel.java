...
public class CinemaUndergroundTunnel extends LevelScript {
    ...
    public void onTurnEnd() {
        if (playerHasWaterBalloon() && !didMessage){
            gi.getTextBox().showMessage("You can throw items. Hold " + getInput(InputMap.THROW_ITEM) + " to get ready to throw the weapon you are currently holding, then press " + getInput(InputMap.ATTACK) + " to throw it.<np>You can also throw <cy>any item from your inventory<cw> by pressing " + getInput(InputMap.THROW_ITEM) + " while hovering over the desired item (while you inventory is open).");
            didMessage = true;
        }
    }
