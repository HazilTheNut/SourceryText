...
public class QuickMenu implements MouseInputReceiver {
    ...
    public QuickMenu(LayerManager lm, Player player, int menuWidth){
        MENU_WIDTH = menuWidth;

        menuLayer = new Layer(MENU_WIDTH, 10, getClass().getSimpleName(), 0, 0, LayerImportances.QUICKMENU);
        menuLayer.fixedScreenPos = true;
        menuLayer.setVisible(false);
        lm.addLayer(menuLayer);

        selectorLayer = new Layer(MENU_WIDTH, 1, getClass().getSimpleName() + "_cursor", 0, 0, LayerImportances.QUICKMENU_CURSOR);
    ...
    public boolean onMouseMove(Coordinate levelPos, Coordinate screenPos) {
        if (menuLayer.getVisible() && !prevPos.equals(screenPos)) {
            ...
            if (!menuLayer.isLayerLocInvalid(layerPos)){
                if (layerPos.getY() > 0 && layerPos.getY() < menuItems.size() + 1) {
